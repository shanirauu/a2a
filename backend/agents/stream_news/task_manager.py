...
from common.types import (
    SendTaskRequest, SendTaskResponse, TaskSendParams, TaskState, TaskStatus,
    Message, Artifact, TextPart, InternalError, InvalidParamsError, JSONRPCResponse,
    SendTaskStreamingRequest, SendTaskStreamingResponse, TaskArtifactUpdateEvent,
    TaskStatusUpdateEvent, Task, PushNotificationConfig
)
...
class AgentTaskManager(InMemoryTaskManager):
    def __init__(self, agent: StreamNewsAgent, notification_sender_auth: PushNotificationSenderAuth):
        super().__init__()
        self.agent = agent
    ...
    async def _run_streaming_agent(self, request: SendTaskStreamingRequest):
        ...
        try:
            async for item in self.agent.stream(query, task_send_params.sessionId):
                ...
                artifact = Artifact(parts=parts, index=item.get("index", 0)) if not end_stream else None
    ...
    def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> JSONRPCResponse | None:
        ...
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes, StreamNewsAgent.SUPPORTED_CONTENT_TYPES
        ):
            return utils.new_incompatible_types_error(request.id)
        if task_send_params.pushNotification and not task_send_params.pushNotification.url:
            return JSONRPCResponse(id=request.id, error=InvalidParamsError(message="Push notification URL missing"))
        return None
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        if request.params.pushNotification:
            if not await self.set_push_notification_info(request.params.id, request.params.pushNotification):
                return SendTaskResponse(id=request.id, error=InvalidParamsError(message="Invalid push URL"))
    ...
    def _get_user_query(self, task_send_params: TaskSendParams) -> str:
        ...
        if not isinstance(part, TextPart):
            raise ValueError("Only text input supported.")
        return part.text
