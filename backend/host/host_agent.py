...
"""
  ...
  async def send_task(
    ...
    """Sends a task to a remote agent, supporting both streaming and non-streaming.
    ...
    request: TaskSendParams = TaskSendParams(
        ...
        pushNotification=PushNotificationConfig(url="http://localhost:9000/notify"),
        metadata={'conversation_id': sessionId},
    )
    response = []
    if agent_name == "Stream News Agent":
        result = await client.send_task(request, self.task_callback)
        if isinstance(result, TaskStatusUpdateEvent):
            ...
            elif result.status.state in [TaskState.CANCELED, TaskState.FAILED]:
                state['session_active'] = False
                raise ValueError(f"Agent {agent_name} task {taskId} {result.status.state.lower()}")
            elif result.status.state == TaskState.INPUT_REQUIRED:
                tool_context.actions.skip_summarization = True
                tool_context.actions.escalate = True
        else:
            raise ValueError(f"Unexpected result type for streaming: {type(result)}")
    else:
